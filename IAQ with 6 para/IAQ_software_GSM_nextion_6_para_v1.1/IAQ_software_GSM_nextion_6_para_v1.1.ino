...
void setup() {
  ...
  EEPROM.get(TEMP_SHIFT_ADDR, tempShift);
  Rtc.Begin();
  Serial.begin(9600);
  SdInit(ssPin);
  Message = "\nstart up---\n" + getDateTimeNow() + "\n";
  ...
  if (EEPROM.read(TEMP_SHIFT_ADDR) == 255) EEPROM.write(TEMP_SHIFT_ADDR, 0);
  tempShift = EEPROM.read(TEMP_SHIFT_ADDR);
  ...
  if (!softFlag) {
    ...
    if (millis() - lt > 7500) {
      ...
      if (ask_pm) {
        ...
        temp = temp - tempShift;
        Serial.print("temp Shift: "); Serial.println(tempShift);
          para_val[TEM] = temp;
          para_val[HUM] = hum;
